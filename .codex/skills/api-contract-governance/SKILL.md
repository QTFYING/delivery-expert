...
2. `packages/types/src/enums` 定义稳定枚举值。
...
3. 涉及枚举值时：再改 `packages/types/src/enums`，收口稳定英文值。
...
- 不要把 `docs/api` 写成第二份 Swagger，也不能让 API 文档再次承担 `contracts` 的结构投影功能；纯结构参数表、响应字段表、分页包装镜像应逐步收回到 Swagger / DTO / contracts 层。
- 禁止重建 `packages/types/src/types`。

## 完成标准

- `docs/api` 可直接供前端理解业务语义、状态流转、字段含义与边界；请求/响应结构、分页包装、`nullable` 与示例以 Swagger / `contracts` 为准。
- 业务枚举闭集的稳定值只在 `packages/types/src/enums` 定义一次；Prisma enum 仅作持久化表达，并通过 mapper 与业务 enum 保持闭集映射一致。
